image_name := env("BUILD_IMAGE_NAME", "zububu")
...
build-containerfile $image_name=image_name:
    sudo {{container_runtime}} build --build-arg DESKTOP=zirc --security-opt label=type:unconfined_t --security-opt apparmor=unconfined  -f Containerfile -t "${image_name}:latest" .
